...
const SuspendUserAccountPage = () => {
  ...
  return (
    <div className="App">
      ...
      <div className="ml-[44rem] text-[20px] font-bold py-8">Admin user account SUSPEND page</div>
      <div className="box">
        <h2 className="pb-7">User Suspension</h2>
        <div className="ml-[43rem] pb-7">
          <label className="pr-7">Select user to suspend:</label>
          <select value={selectedUser} onChange={handleSelectUser} required className="rounded-full px-10">
        ...
        <div className="ml-[47rem] gap-7 flex py-10">
          ...
          <button onClick={handleSuspendUser} className="px-4 py-2 border border-gray-900 rounded-full bg-green-500 text-[20px] text-white hover:bg-green-700">
            Suspend
          </button>
          <Link to={"/adminUserAccountPage"}>
            <button className="px-4 py-2 border border-gray-900 rounded-full bg-red-500 text-[20px] text-white hover:bg-red-700">Back</button>
          </Link>
        </div>
      </div>
    </div>
